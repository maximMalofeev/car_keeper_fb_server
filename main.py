...
def on_snap_in_danger_col(col_snap, changes, read_time):
    for change in changes:
        if change.type.name == "ADDED":
            print(u'{} => {}, change type = {}'.format(
                change.document.id, change.document.to_dict(), change.type))
...
def notify_owners(owners):
    ...
    for owner in owners:
        ...
        if user_snap.exists:
            ...
            print(
                f"Send notification to {owner} with name: {user_dict['name']}")
            # TODO send multicast message
            send_notification(title="Your car in danger",
                              body="Harry up!", token=user_dict["registration_token"])
        else:
...
if __name__ == "__main__":
    try:
        ...
        db = firestore.client()
        col_watch = db.collection(
            'in-danger').on_snapshot(on_snap_in_danger_col)
